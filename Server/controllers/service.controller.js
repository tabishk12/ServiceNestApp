...
const AvailableServices = async (req, res) => {
  try {
    ...
    const registeredIds =
      user.spDetails?.map((sp) => sp.serviceId._id.toString()) || [];
...
const RegisterService = async (req, res) => {
  try {
    ...
    const alreadyAdded = user.spDetails?.some(
      (sp) => sp.serviceId.toString() === serviceId,
    );
    if (alreadyAdded) {
...
export { AvailableServices, RegisterService, DeleteService };
